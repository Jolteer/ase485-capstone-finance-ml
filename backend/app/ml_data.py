...
Pure data module; no behavior. Kept separate from ``ml_engine`` so the engine
file stays focused on logic. Extend by adding more (description, label) pairs
- the model is re-trained in-memory on startup.
"""
